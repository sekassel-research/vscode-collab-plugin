...
export function buildCursorMovedMessage(operation: string, pathName: string, cursor: vscode.Position, selectionEnd: vscode.Position, name: string | undefined, project: string | undefined) {
    return JSON.stringify({operation, data: {pathName, cursor, selectionEnd, name, project}});
}

export function buildTextReplacedMessage(operation: string, pathName: string, from: vscode.Position, to: vscode.Position, content: string, name: string | undefined, project: string | undefined) {
